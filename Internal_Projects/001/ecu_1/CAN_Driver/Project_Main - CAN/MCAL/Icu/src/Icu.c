...
#define ICU_SW_PATCH_VERSION_C              4
...
*/
static VAR(Mcl_DmaTcdAttributesType, AUTOMATIC) starttimestamp_tcd_config[ICU_MAX_CHANNEL];
...
*           defined at block scope
*/
static VAR(Mcl_DmaTcdAttributesType, AUTOMATIC) startsignalmeasurement_tcd_config[ICU_MAX_CHANNEL];
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
               ...
               )
            {
                ...
                if(ICU_NOMCLDMACHANNEL != (*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel)
                {
                    ...
                    starttimestamp_tcd_config[Channel].u32saddr = Icu_Ipw_GetStartAddress (Channel, &Icu_pCfgPtr->IpConfig);
                    ...
                    starttimestamp_tcd_config[Channel].u32daddr = (uint32)BufferPtr;
                    starttimestamp_tcd_config[Channel].u32ssize = (uint32)ICU_DMA_SIZE_U32;
                    starttimestamp_tcd_config[Channel].u32dsize = (uint32)ICU_DMA_SIZE_U32;
                    starttimestamp_tcd_config[Channel].u32soff  = (uint32)0;
                    starttimestamp_tcd_config[Channel].u32doff  = (uint32)ICU_DMA_OFFSET_U32;
                    starttimestamp_tcd_config[Channel].u32smod  = (uint32)0;
                    starttimestamp_tcd_config[Channel].u32dmod  = (uint32)0;
                    if ((NotifyInterval >= BufferSize) || (NotifyInterval == (uint16)0))
                    {
                        starttimestamp_tcd_config[Channel].u32num_bytes = ICU_DMA_NUM_BYTES_U32;
                        starttimestamp_tcd_config[Channel].u32iter = BufferSize;
                    }
                    else if (NotifyInterval < BufferSize)
                    {
                        starttimestamp_tcd_config[Channel].u32num_bytes = ICU_DMA_NUM_BYTES_U32;
                        starttimestamp_tcd_config[Channel].u32iter = NotifyInterval;
                    }
                    else
                    {
                        /* does nothing*/
                    }
                    Mcl_DmaConfigChannel((*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel, &starttimestamp_tcd_config[Channel]);
    ...
    if((Std_ReturnType)E_OK == Icu_ValidateModeForSetUpFeature(ICU_STARTSIGNALMEASUREMENT_ID))
    {
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
            ...
            SchM_Enter_Icu_ICU_EXCLUSIVE_AREA_08();
            {
                ...
                {
                    /* initialize the members of the structure */
                    startsignalmeasurement_tcd_config[Channel].u32saddr = Icu_Ipw_GetStartAddress (Channel, &Icu_pCfgPtr->IpConfig);
                    /* @violates @ref Icu_c_REF_7 Violates MISRA 2004 Rule 11.1, Cast from unsigned long to pointer.*/
                    /* @violates @ref Icu_c_REF_10 Violates MISRA 2004 Rule 11.3, Cast from pointer type and integral type.*/
                    /* Compiler_Warning: This warning is thrown because of a conversion between a Icu_ValyeType pointer and a uint32 */
                    startsignalmeasurement_tcd_config[Channel].u32daddr = (uint32)&Icu_aDmaBuffer[Channel][0];
                    startsignalmeasurement_tcd_config[Channel].u32ssize = (uint32)ICU_DMA_SIZE_U32;
                    startsignalmeasurement_tcd_config[Channel].u32dsize = (uint32)ICU_DMA_SIZE_U32;
                    startsignalmeasurement_tcd_config[Channel].u32soff  = (uint32)0;
                    startsignalmeasurement_tcd_config[Channel].u32doff  = (uint32)ICU_DMA_OFFSET_U32;
                    startsignalmeasurement_tcd_config[Channel].u32smod  = (uint32)0;
                    startsignalmeasurement_tcd_config[Channel].u32dmod  = (uint32)ICU_DMA_MAJORLOOP_COUNT;
                    startsignalmeasurement_tcd_config[Channel].u32num_bytes = ICU_DMA_NUM_BYTES_U32;
                    startsignalmeasurement_tcd_config[Channel].u32iter = ICU_DMA_MAJORLOOP_FIRST_COUNT;
                    Mcl_DmaConfigChannel((*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel, &startsignalmeasurement_tcd_config[Channel]);
                    Mcl_DmaEnableHwRequest((*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel);
                    Mcl_DmaEnableNotification((*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel, MCL_DMA_TRANSFER_COMPLETE);
