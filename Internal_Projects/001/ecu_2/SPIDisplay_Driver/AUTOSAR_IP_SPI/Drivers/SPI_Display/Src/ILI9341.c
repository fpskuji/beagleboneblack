...
volatile uint16 LCD_HEIGHT = ILI9341_SCREEN_HEIGHT;
volatile uint16 LCD_WIDTH = ILI9341_SCREEN_WIDTH;

int isClearText = FALSE;
...
FUNC(void, AUTOMATIC) DrawPixel(uint16 x, uint16 y)
{
    ...
    uint8 bufferC[2] = {GLOBAL_FONT_COLOR >> 8, GLOBAL_FONT_COLOR};
    ...
    if (isClearText == TRUE)
    {
        uint8 bufferC[2] = {GLOBAL_BG_COLOR >> 8, GLOBAL_BG_COLOR};
